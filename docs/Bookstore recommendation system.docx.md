...
The Flask website follows the SQLite runtime path shown above. The submitted demo focuses on one backend and one frontend: Flask handles data/model/database logic, while the HTML/CSS interface handles user interaction.
...
* The evaluation metrics below are retained as project results, but the runnable demo only requires the Flask backend and frontend.
* `src/booklog/web.py`: Initializes SQLite from the processed CSV files when necessary, loads runtime data, trains and caches the model, handles rating updates and serves the bookstore interface. The root `app.py` file is a thin entrypoint for running the website.
...
The evaluation was run during development on the processed dataset. The final demo is launched with:
