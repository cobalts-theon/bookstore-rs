...
`Bookstore recommendation system.docx.md`.
...
- Testing: pytest.
- Dataset: Kaggle Book Recommendation Dataset; `data/demo` dùng để chạy thử nhanh.
...
1. **Data pipeline (`src/booklog/data.py`)**  
...
2. **Content-based model (`src/booklog/content.py`)**  
...
3. **SQLite persistence (`src/booklog/database.py`)**  
...
4. **Collaborative model (`src/booklog/collaborative.py`)**  
...
5. **Dynamic hybrid (`src/booklog/hybrid.py`)**  
...
6. **Evaluation (`src/booklog/evaluation.py`)**  
   Tách leave-one-out theo từng user, đo RMSE/MAE cho rating prediction và
   HitRate@K/Precision@K cho Top-N recommendation.
...
Kết quả kiểm tra khả năng chạy end-to-end trên `data/demo`:
...
Đây là kết quả smoke-test trên dữ liệu demo nhỏ, không phải kết quả nghiên cứu
cuối cùng. Kết quả chính thức cần chạy trên `data/raw` bằng:

```powershell
python cli.py --data-dir data/raw evaluate --top-k 10
```
